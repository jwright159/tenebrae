...
public class EntityBox extends Table{
	...
	public static class TextBar extends Stack{
		...
		public TextBar(String text, float min, float max, float step, boolean vertical, Skin skin, String skinName){
			this(text, min, max, step, vertical, skin.get(skinName, ProgressBar.ProgressBarStyle.class), skin.get("bar", Label.LabelStyle.class));
		}
		public TextBar(String text, float min, float max, float step, boolean vertical, Skin skin){
			this(text, min, max, step, vertical, skin.get(vertical ? "default-vertical" : "default-horizontal", ProgressBar.ProgressBarStyle.class), skin.get("bar", Label.LabelStyle.class));
		}
		public TextBar(String text, float min, float max, float step, boolean vertical, ProgressBar.ProgressBarStyle style, Label.LabelStyle labelStyle){
			setDebug(Tenebrae.TABLEDEBUG);
			this.add(bar = new SizableProgressBar(min, max, step, vertical, style));
			this.add(new Container<Label>(this.text = new Label(text, labelStyle)).padLeft(Tenebrae.MARGIN / 2f).padRight(Tenebrae.MARGIN / 2f).fill());
